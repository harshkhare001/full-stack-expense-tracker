...
async function addItem(e){
  ...
  try
  {
    ...
    await axios.post("http://65.0.87.252:3000/addExpense",expense,{ headers: {Authorization : token}});
...
async function renderList() {
  ...
  try {
    ...
    const res = await axios.get(`http://65.0.87.252:3000/expenses?page=${page}&limit=${limit}`,{ headers: { Authorization: token }});
...
list.addEventListener("click", async function del(e){
  ...
  if (e.target.classList.contains("delete")) {
    ...
    let id = e.target.id;
    await axios.get(`http://65.0.87.252:3000/deleteExpense/${id}`);
    list.removeChild(li);
  }
...
{
  const token = localStorage.getItem('token');
  const response = await axios.get('http://65.0.87.252:3000/prememiumuser',{headers: {Authorization:token}});
  ...
  var options = 
  {
    ...
    handler: async function (response){
      const res = await axios.post('http://65.0.87.252:3000/updatetransaction',
        {
...
document.getElementById('lead-button').onclick = async function(e)
{
  ...
  table.innerHTML=``;
  const token = localStorage.getItem('token');
  const response = await axios.get("http://65.0.87.252:3000/premium/getleaders",{ headers : { Authorization : token }});
  console.log(response);
